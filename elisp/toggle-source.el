...
take you to the buffer \"file.h\" - assuming this file existed."
  ...
  (let ((filename (buffer-file-name))
    ...
    (while (and list (not found) filename)
      (setq entry (car list))
      (setq list  (cdr list))
      (setq toggled-filename nil)
      (if (string-match (concat (regexp-quote (car entry)) "$") filename)
            (setq toggled-filename (cdr entry)))
      (if (string-match (concat (regexp-quote (cdr entry)) "$") filename)
            (setq toggled-filename (car entry)))
      (if (and toggled-filename (file-exists-p (concat (substring filename 0 (match-beginning 0)) toggled-filename)))
          (progn
            (find-file (concat (substring filename 0 (match-beginning 0)) toggled-filename))
            (setq found t)))))))
...
(global-set-key (kbd "C-x z") 'toggle-source)
